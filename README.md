...
| ระยะทาง | 10 | *ย้ายไปคิดหลัง base — ดูด้านล่าง* |
...
**ระยะทางไม่ได้ถูกตัดทิ้ง — ถูกย้ายไปคิดทีหลัง**

ระยะทางยังมีน้ำหนัก **10 เท่าเดิมทุกประการ** สิ่งที่เปลี่ยนคือ *ลำดับ* การคำนวณ ไม่ใช่น้ำหนัก
คอลัมน์ "หลัง normalize" ข้างบนคือสัดส่วนของ 11 หมวดที่เหลือในขั้น `base` เท่านั้น

คะแนนจึงถูกคำนวณสองรอบ (`backend/src/matching/score.js`):

| ขั้น | คิดจาก | ความหมาย |
|---|---|---|
| `base_mutual_fit` | 11 หมวด (ไม่รวมระยะทาง) normalize เป็น 100 | **"คนนี้ตรงใจคุณแค่ไหน ถ้ายังไม่คิดว่าอยู่ไกลแค่ไหน"** |
| `final_mutual_fit` | นำระยะทางกลับเข้ามาที่น้ำหนัก 10 เดิม | **คะแนนรวมจริงเมื่อรวมความไกลด้วย** |

**ทำไมต้องแยกสองรอบ** — เหตุผลทางเทคนิคคือ V5 §20.3 กำหนดให้ทดสอบเกณฑ์ exceptional match
กับคะแนน *"ก่อน distance penalty"* ซึ่งทำไม่ได้ถ้าระยะทางฝังอยู่ในคะแนนเดียว

แต่เหตุผลที่แท้จริงคือเรื่องของผู้ใช้ — ถ้าระยะทางฝังอยู่ในคะแนนเดียว
ผู้ดูแลที่ **ตรงใจครอบครัวที่สุดแต่อยู่ไกล** จะถูกคะแนนระยะทางกดจนตกอันดับหายไปเงียบ ๆ
ครอบครัวจะไม่มีวันได้เห็นเขา — เท่ากับระบบตัดสินใจแทนไปแล้วโดยที่ครอบครัวไม่รู้ตัว

การแยก `base` ออกมาทำให้เก็บคนกลุ่มนั้นไว้ได้ แล้วนำมาวางเทียบเป็นสองทางเลือกให้ครอบครัว
**เลือกเอง** (`backend/src/services/distanceOptions.js`):

| ทางเลือก | สิ่งที่ได้ | สิ่งที่ต้องจ่ายเพิ่ม |
|---|---|---|
| **เลือกคนที่อยู่ใกล้** | อยู่ในระยะที่ตกลงไว้ | ไม่มี |
| **เลือกคนที่เหมาะที่สุด แม้อยู่ไกล** | ความเข้ากันสูงกว่า | ค่าเดินทาง + อาจมีค่าที่พัก และต้องตกลงกันทั้งสองฝ่าย |

ระบบแสดงส่วนต่างให้เห็นตรง ๆ เช่น *"คนที่อยู่ไกลเหมาะกว่า 12 คะแนน แต่มีค่าใช้จ่ายเพิ่มประมาณ 2,400 บาท"*
แล้ว **หยุดตรงนั้น** — ไม่มีบรรทัดใดในโค้ดที่เลือกแทนครอบครัว
ฝั่งผู้ดูแลเห็นดีลเดียวกันจากอีกด้าน (`caregiverDistanceNote`) ว่างานอยู่นอกพื้นที่
ค่าเดินทางเป็นเพียงประมาณการ ต้องตกลงกับครอบครัวก่อนจึงรับงานได้
